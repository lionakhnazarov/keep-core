...
# Script to register all operators for multi-node setup using keep-client CLI
# This registers operators in both RandomBeacon and WalletRegistry
...
for config_file in "${CONFIG_FILES[@]}"; do
    ...
    if [ -z "$NODE_NUM" ]; then
        echo "⚠️  Could not extract node number from $config_file"
        continue
    fi
    ...
    if [ -z "$KEYFILE" ]; then
        echo "⚠️  KeyFile not found in $config_file"
        continue
    fi
    ...
    if [ ! -f "$KEYFILE" ]; then
        echo "⚠️  KeyFile not found: $KEYFILE"
        continue
    fi
    ...
    if [ -z "$OPERATOR" ] || [ "$OPERATOR" = "null" ]; then
        echo "⚠️  Could not extract address from $KEYFILE"
        continue
    fi
...
# Register each operator in both RandomBeacon and WalletRegistry
SUCCESS_COUNT=0
FAIL_COUNT=0
...
for i in "${!OPERATORS[@]}"; do
    ...
    echo "--- Registering Node $NODE_NUM ($OPERATOR) ---"
    
    # Register in RandomBeacon
    echo "  Registering in RandomBeacon..."
    if KEEP_ETHEREUM_PASSWORD="$PASSWORD" ./keep-client ethereum beacon random-beacon register-operator \
        "$OPERATOR" \
        --submit \
        --config "$CONFIG_FILE" \
        --developer 2>&1 | grep -vE "(You are using a version|Please, make sure|To learn more|Error encountered|No need to generate|Contract Name|Size \(KB\)|^ ·|^ \||^---)" | grep -E "(Transaction|hash|SUCCESS|Error|registered|already)" | head -3; then
        echo "    ✓ RandomBeacon registration submitted"
    else
        echo "    ⚠ RandomBeacon registration may have failed"
    fi
    
    sleep 1
    
    # Register in WalletRegistry
    echo "  Registering in WalletRegistry..."
    if KEEP_ETHEREUM_PASSWORD="$PASSWORD" ./keep-client ethereum ecdsa wallet-registry register-operator \
        "$OPERATOR" \
        --submit \
        --config "$CONFIG_FILE" \
        --developer 2>&1 | grep -vE "(You are using a version|Please, make sure|To learn more|Error encountered|No need to generate|Contract Name|Size \(KB\)|^ ·|^ \||^---)" | grep -E "(Transaction|hash|SUCCESS|Error|registered|already)" | head -3; then
        echo "    ✓ WalletRegistry registration submitted"
        SUCCESS_COUNT=$((SUCCESS_COUNT + 1))
    else
        echo "    ⚠ WalletRegistry registration may have failed"
...
if [ $FAIL_COUNT -eq 0 ]; then
    ...
    echo "  KEEP_ETHEREUM_PASSWORD=password ./keep-client ethereum beacon random-beacon register-operator <OPERATOR> --submit --config configs/node<N>.toml --developer"
